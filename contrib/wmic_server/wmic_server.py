...
config_file=os.getenv('WMIC_SERVER_CONFIG',str(working_dir) + '/wmic_server.yaml')
   ...
   try:
      for regex in regex_list:
         if re.search(regex,param_value,flags=re.IGNORECASE) is not None:
            # found a valid match
            debug and print(param_name + "=" + param_value + " matches regex " + regex)
            return param_value
   except:
      return param_value
...
def wmic():
   ...
   s=asyncio.run(wmic_core())
   return s
# -------------------------------------------------------------------------
async def wmic_core():
   global debug

   # load the parameters from the request or set them to default value if not present
   id=validate_input(request.method,'id','',cfg['validation']['id'])
   token=validate_input(request.method,'token','',cfg['validation']['token'])
   host=validate_input(request.method,'host','',cfg['validation']['host'])
   query_input=validate_input(request.method,'query','',cfg['validation']['query'])
   namespace=validate_input(request.method,'namespace','root/cimv2',cfg['validation']['namespace'])  # defaulting to root/cimv2
   
   valid_token=False # flag to tell us if the http call contains a valid token
   token_auth=False # flag to tell us if we are using token auth ie there are some tokens defined
   ...
   if not valid_token:
      try:
         # now see if the token is one of the valid tokens in our config list for this id
   ...
   if (token_auth and valid_token) or (not token_auth):
      # load the user/pass out put the yaml config file based off the id value passed
      ...
      try:
        ...
        return out
         ...
         debug and print(str(error))
         return "Problem with the wmi query on the target host. " + str(error)
      finally:
         if service:
            service.close()
         conn.close()
...
# in production run like 
# gunicorn -b 127.0.0.1:2313 --pythonpath /opt/nagios/bin/plugins/aiowmi,/opt/nagios/bin/plugins/aiowmi/contrib/wmic_server --threads 1 wmic_server:app

# for development run like:
# optionally insert the following variables into the command line if needed: WMIC_SERVER_ADDRESS WMIC_SERVER_PORT
# PYTHONPATH=/opt/nagios/bin/plugins/aiowmi FLASK_ENV=development WMIC_SERVER_DEBUG=1 python /opt/nagios/bin/plugins/aiowmi/contrib/wmic_server/wmic_server.py
